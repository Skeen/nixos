{impermanence, ...}: {
  ...
  # wiped every reboot (here by rolling the btrfs @root subvolume back to a
  # pristine snapshot in the initrd, see ./disko.nix). Such a setup is possible
  # because NixOS only needs /boot and /nix in order to boot, all other system
  # files are simply links to files in /nix.

  # The impermanence module bind-mounts persistent files and directories,
  # stored in /nix/persist, into the wiped root partition on startup. For
